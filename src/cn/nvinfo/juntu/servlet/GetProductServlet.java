		...
		try {
			...
			for (int i = 0; i < ja.length(); i++) {
				...
				if(ja.getJSONObject(i).getInt("status")==1){
					...
					//将visitorMode存入数据库
					com.alibaba.fastjson.JSONObject visitorInfoRule_Object=com.alibaba.fastjson.JSONObject.parseObject(visitorInfoRule);
					
					product.setVisitorMode(visitorInfoRule_Object.getInteger("visitorMode"));
					log.info("即将存入数据库的visitorMode="+ visitorInfoRule_Object.getInteger("visitorMode"));
					...
					if(product_old!=null){
						...
						juntuService.editProduct(product);
					}else{
