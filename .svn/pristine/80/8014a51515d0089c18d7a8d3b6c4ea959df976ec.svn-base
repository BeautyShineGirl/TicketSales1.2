...
public class CustomDaoImp implements CustomDao {
	...
	public List<Custom> getUrlList(Integer id) {
		return template.selectList("custom.getUrlList",id);
	}
	public String getUrl(Integer id) {
		return template.selectOne("custom.getUrl",id);
	}
	//将验证码存入数据库	杨立	2018-01-30
	public int addCode(String code, String userName) {
		Map<String,Object> map=new HashMap<String, Object>();
		map.put("code", code);
		map.put("userName", userName);
		return template.update("custom.addCode", map);
	}
	//查询库中code	2018-01-30	杨立
	public String findCode(String userName) {
		return template.selectOne("custom.findCode", userName);
	}
